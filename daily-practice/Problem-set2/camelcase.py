...
whereby the first letter of the first word is lowercase but the first letter of each subsequent word is uppercase. For instance, 
whereas a variable for a user’s name might be called name, a variable for a user’s first name might be called firstName, 
...
def transform(camelcase):
    resultado = ""
    for letra in camelcase:
        if letra.isupper():
            resultado = resultado + "_" + letra.lower()
        else:
...
camelcase = str(input("ingresa el nombre en camelCase: "))

print(transform(camelcase))
